...
async def get_review_queue(
    ...
    try:
        ...
        count_result = db.query(f"SELECT count() FROM source WHERE status = '{status}' GROUP ALL;")
        total = 0
        ...
        if action.action == "approve":
            db.query(f"""
                UPDATE {action.source_id} SET
                    ...
                    updated_at = time::now();
            """)
            logger.info(f"Approved: {action.source_id} by {user.username}")
            ...
            db.query(f"""
                UPDATE {action.source_id} SET
                    ...
                    updated_at = time::now();
            """)
            logger.info(f"Rejected: {action.source_id} by {user.username}")
...
async def batch_review(
    ...
    try:
        ...
        logger.info(f"Batch review: {len(results)} documents by {user.username}")
...
@router.get("/stats")
async def curation_stats(user: User = Depends(get_current_user)):
    """策展统计：各状态文档数量。"""
    db = get_db()
    try:
        result = db.query("""
